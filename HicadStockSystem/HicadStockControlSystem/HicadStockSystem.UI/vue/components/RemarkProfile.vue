    ...
    <form @submit.prevent="checkForm" method="post">
      <div class="p-5" id="vertical-form">
        <div class="preview">
          <div class="row">
            <div class="col-3">
              <label for="supplierCode" class="mb-1">Remark</label>
              <input
                class="form-control"
                name="supplierCode"
                v-model="postBody.remark"
                v-bind:class="{
                  'form-control': true,
                  'is-invalid': !remarksIsValid && codeblur,
                }"
...
export default {
  ...
  methods: {
    ...
    postPost() {
      if (this.submitorUpdate == "Submit") {
        axios
          .post(`/api/remarks/`, this.postBody)
          .then((response) => {
  ...
  computed: {
    remarksIsValid() {
      return (
        this.postBody.remark != "" &&
        this.postBody.remark.length >= 1 &&
        this.postBody.remark.length <= 50
      );
    },
    ...
    setter() {
      ...
      if (objecttoedit.id) {
        this.postBody.remark = objecttoedit.remark;
      }
    },
  },
};
